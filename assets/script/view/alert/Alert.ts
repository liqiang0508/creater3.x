...
export default class Alert extends popBaseView {
    ...
    setTitle(str) {//设置a提示文字
        this.mChild.text.getComponent(Label).string = str;
    }
    ...
    setButtonText(btn, text) {
        var lable = btn.getChildByName("Label")
        if (lable) {
            lable.getComponent(Label).string = text
        }
